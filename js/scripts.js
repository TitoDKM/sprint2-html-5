...
function handleSearch(e) {
	...
	for(let i = 0; i < students.length; i++) {
		let hasMatch = false;
		let studentName = students[i].getElementsByTagName("th")[0].innerText;

		if(studentName.toLowerCase().indexOf(filter) > -1)
			hasMatch = true;

		let otherFields = students[i].getElementsByTagName("td");
		for(let j = 0; j < otherFields.length; j++) {
			const fieldText = otherFields[j].innerText.toLowerCase();
